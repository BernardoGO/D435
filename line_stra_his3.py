...
figtext(.5,.95,'Grupo:' + str(grupo) + ' Logaritmico', fontsize=14, ha='center')
figtext(.5,.93,"NG: " + str(numcons),ha='center',fontsize=10 )
figtext(.5,.91,'Estrato 1 - Consumidores de Grande Porte',fontsize=10,ha='center')# NE 1:'+str(len(strMaior)/lendias)
#ax.set_ylabel('FC')
plt.plot(c,maiorlvlsMa, label='Maximo', marker='.', color="#FF0000", alpha=0 )
plt.plot(c,totDias1, label='Media E.1', marker='+', color="#000000", alpha=0 )
plt.plot(c,totDiasTod , label='Media Geral', marker='.', color="#5B913C", linewidth=0.5, alpha=0)
width=0.2
steps = max(coday2 + coday1) / 20;

plt.plot(c,menorlvlsMa , label='Minimo', marker='x', color="#0000FF", alpha=0)
...
ax22.set_ylabel('Cons')
ax22.set_yscale('log')
loc = ticker.MaxNLocator(nbins=8) # this locator puts ticks at regular intervals

ax22.yaxis.set_major_locator(loc)
start, end = ax22.get_ylim()
try:
	ax22.yaxis.set_ticks(np.arange(start, end, steps))
except:
	pass
loc = ticker.MaxNLocator(nbins=8) # this locator puts ticks at regular intervals
...
ax22.bar(c, coday1, alpha=0.2, log = True)
ax22.yaxis.tick_left()
plt.tick_params(axis='y', labelsize=3)
ax22.set_ylim(1, max(coday2 + coday1))
...
ax22.yaxis.set_major_formatter(matplotlib.ticker.ScalarFormatter())
ax22.yaxis.set_major_formatter(matplotlib.ticker.FormatStrFormatter('%d'))
ax22.yaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(lambda x, pos: str(int(round(x)))))
if max(coday2 + coday1) > 8:
	loc = ticker.MaxNLocator(nbins = 8) # this locator puts ticks at regular intervals
else:
	loc = ticker.MaxNLocator(nbins =  max(coday2 + coday1)-1)
ax22.yaxis.set_major_locator(loc)
...
figtext(.5,.51,'Estrato 2 - Consumidores de Pequeno Porte', fontsize=10, ha='center')
plt.plot(c,maiorlvlsMe, label='Maximo', marker='.', color="#FF0000", alpha=0 )
plt.plot(c,totDias2, label='Media E.2', marker='+', color="#000000", alpha=0 )
width=0.2
sume = sume+1
#plt.plot(c,totDias1 , label='media E.1', marker='.', color="#FFE4B5" , linewidth=0.5 )
plt.plot(c,totDiasTod , label='Media Geral', marker='.', color="#5B913C", linewidth=0.5, alpha=0)
plt.plot(c,menorlvlsMe , label='Minimo', marker='x', color="#0000FF", alpha=0)
...
#ax221.set_xlim(x.min(), x.max())
ax221.set_ylabel('Cons')


ax221.set_ylim(1, max(coday2 + coday1))
#ax221.get_yaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
#ax221.get_yaxis().get_major_formatter().labelOnlyBase = True

ax221.yaxis.tick_left()
start, end = ax221.get_ylim()
try:
	ax221.yaxis.set_ticks(np.arange(start, end, steps))
except:
	pass

ax221.bar(c, coday2, alpha=0.2, log = True)

print bcolors.BACK_LRED + " OK! " + bcolors.ENDC
plt.tick_params(axis='y', labelsize=3)
ax221.yaxis.set_major_formatter(matplotlib.ticker.ScalarFormatter())
ax221.yaxis.set_major_formatter(matplotlib.ticker.FormatStrFormatter('%d'))
ax221.yaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(lambda x, pos: str(int(round(x)))))

if max(coday2 + coday1) > 8:
	loc = ticker.MaxNLocator(nbins = 8) # this locator puts ticks at regular intervals
else:
	loc = ticker.MaxNLocator(nbins =  max(coday2 + coday1)-1)
ax221.yaxis.set_major_locator(loc)


subplots_adjust(bottom=0.14)
plt.savefig(str(grupo)+'grpLOG.pdf', bbox_inches=None, pad_inches=0.1, frameon=None)
