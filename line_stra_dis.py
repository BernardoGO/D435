...
plt.tick_params(axis='x',which='both',      # both major and minor ticks are affected
    ...
    labelbottom='off')
ax22 = ax.twinx()
ax22.bar(c, coday1, alpha=0.2)

if TresGraficos == True:
	#0,21
	ax1 = plt.subplot(3,1,sume+1)
...
#	figtext(.5,.91,'Estrato 1 - Consumidores de Grande Porte  N.:'+str(len(strMaior)/lendias),fontsize=10,ha='center')
	...
	if CalcularErro == False:
		plt.plot(c,totDias1, label='maximo', marker='.', color="#FF0000" )
		plt.plot(c,totDias2 , label='minimo', marker='x', color="#0000FF")
		plt.plot(c,totDiasTod, label='media', marker='+', color="#000000" )
	else:
		plt.plot(c,tuple(0 for x in xrange(0, lendias)), color="#AAAAAA" )
		plt.plot(c,totErro1, label='Grande Porte', marker='.', color="#FF0000" )
		plt.plot(c,totErro2 , label='Pequeno Porte', marker='.', color="#0000FF")
